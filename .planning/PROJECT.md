...
### Validated
- ✓ Setup testing framework (M1)
- ✓ Greenhouse discovery + composite key & embedding dedup (M2)
- ✓ Embedding / Keyword Matcher + tailored Generator & GroundednessVerifier (M3)
- ✓ Redis Streams, shadow-worker browser automation (M4)

### Active
- [ ] Implement robust LLM integration strategy with token cost controls and chains (primary/secondary/fallback) across:
  - Job fetching details extraction
  - Semantic/borderline matching
  - Playwright custom questions answering
  - Resume LaTeX tailoring code generation

### Out of Scope
- Direct crawling of unauthorized sources (e.g. LinkedIn/Indeed scrapers)

## Key Decisions
| Decision | Rationale | Outcome |
|----------|-----------|---------|
| Modular Monolith | Single-user portfolio tool doesn't justify microservice overhead | Implemented |
| Redis Streams | Durable local messaging queue between Java and TypeScript | Implemented |
